...
function Laptop() {
    ...
    const { dragon, desktop, laptop, tablets } = useSelector((state) => state?.product[0][0])
    const state = useSelector((state) => state?.product[0][0])
    const dispatch = useDispatch()
    var data2 = (category != '' ? category : id.id.toLowerCase())
    const filterData = id?.id === 'Desktop' ? desktop : ''
    // const filterData = 
    // const datafilter3 = state?.filter((e) => e.category.toLowerCase() == id.id.toLowerCase())
    // const datafilter4 = state?.filter((e) => e.subcategory.toLowerCase() == data2)
...
console.log('propertyNames',propertyNames,findTrue,count);
console.log('statestatestatestate',id);
